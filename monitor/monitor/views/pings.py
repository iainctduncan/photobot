...
def pings_by_install(request):
    ...
    pings = request.dbsession.query(Ping).filter(Ping.installation_uid==install_id).order_by(Ping.datetime.desc()).limit(50).all()
...
def pings_by_subsystem(request):
    ...
    log.info(install_id)
    # the installations shows the latest ping from all active installations
    pings = request.dbsession.query(Ping).filter(Ping.installation_uid==install_id).filter(Ping.subsystem==subsystem).order_by(Ping.datetime.desc()).limit(50).all()
...
def pings(request):
    ...
    # the installations shows the latest ping from all active installations
    pings = request.dbsession.query(Ping).order_by(Ping.datetime.desc()).limit(50).all()
